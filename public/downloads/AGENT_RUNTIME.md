...
Optional `COATRIA_CODEX_MODEL` selects a model available to that worker account for a legacy registration; omission uses the CLI default. For a Plugins installation, the reviewed model in its leased runtime context takes precedence. The persistent company character also accompanies every task, beneath the fixed permissions and AI-identity policy. `COATRIA_CODEX_TIMEOUT_SECONDS` defaults to 600 and accepts 10–1,500 seconds; an installed bridge applies the shorter of this operator limit and its configured deadline. Direct-API step/token limits do not apply to Codex CLI. Provider authentication and usage charges remain with the operator's account. Keep any `CODEX_API_KEY` confined to the trusted worker invocation; do not share it with repository scripts. Cancellation requests termination of the direct Codex process, then forceful termination after two seconds if needed; descendant-process containment still depends on the worker OS or supervisor.

The [plugin directory](https://coatria.com/api/plugins/catalog) publishes versioned bridge metadata. Use [provider setup](https://coatria.com/downloads/PROVIDER_BRIDGES.md) for Claude Code, direct OpenAI/Anthropic/xAI inference, Fireworks/Together Qwen, and operator-managed Runpod endpoints. [Company autopilot](https://coatria.com/downloads/AUTOPILOT.md) explains bounded recurring missions. A running worker now checks approved missions while idle; installing an integration alone does not start background execution. Connection checks have no tool grants or company conversation history. Mission execution failures and expired leases stop for review rather than automatically re-running uncertain reasoning.
