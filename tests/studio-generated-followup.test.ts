...
test('generated continuation executes exact leased tools against immutable synthetic verified sources',{skip:!emulate&&!localPostgres,timeout:180000},async t=>{
 ...
 try{
  ...
  async function fixture(kind:keyof typeof specs='image',options:{budget?:number;optIn?:boolean;reuse?:boolean;staffed?:boolean}={}){
   ...
   if(options.staffed){
    ...
    const prefix=`companies/${company}/studio/staffing/proposals`,proposed=await api(prefix,'POST',{clientId:randomUUID(),templateId:'ai-production',brief:'Three specialized agents for a supervised generated-media company.',teamSize:3,disciplines:['compositing'],reviewerHumanId:null,provider:{pluginId:'runpod',manifestVersion:'1.0.0',runtimeConfig:{providerId:'runpod',modelId:'Qwen/Qwen3.8-27B-FP8'}}},201),plan=proposed.proposal.plan,generation=plan.specialists.find((person:Row)=>person.roleKeys.includes('comp'));
    assert.deepEqual(generation.roleKeys,['comp']);assert.deepEqual([...generation.capabilities].sort(),[...caps].sort());assert.match(generation.character.persona,/reviewed storage.read grant/);assert(!generation.capabilities.includes('storage.organize'));assert(!generation.capabilities.includes('storage.write'));
   ...
   const parent=async(member:Membership)=>{const queued=await transaction(c=>createAgentRunInTransaction(c,member,'commons',{clientId:randomUUID(),agentId:coordinator.id,prompt:'Coordinate the exact synthetic source.'}));const lease=await claim(coordinator);assert.equal(lease.run.id,queued.run.id);return lease;};
   await approve(memberA,false,0);const firstParent=await parent(memberA);
   const initial=(await tool(coordinator,firstParent,'studio_work_dispatch',{projectId:p.id,workItemId:work.id,projectRevision:await projectRevision(),policyRevision:1})).result as Row,source=await claim(specialist);assert.equal(source.run.id,initial.childRunId);
   await tool(specialist,source,'tasks_claim',{taskId:work.task_id,revision:(await task()).revision});
   const sourceTask=await task();
   ...
   await approve(memberB,options.optIn!==false,1);const parentLease=await parent(memberB);
  ...
  for(const kind of['image','video','audio'] as const)await t.test(kind+' staffing-created specialist claims, registers and submits exactly once even with new outer transport IDs',async()=>{
   const f=await fixture(kind,{staffed:true}),queued=(await f.dispatch()).result as Row;assert(queued.childRunId);assert.notEqual(queued.childRunId,f.source.run.id);assert.equal((await f.policy()).runs_started,2);assert.equal(((await f.dispatch()).result as Row).childRunId,queued.childRunId);
