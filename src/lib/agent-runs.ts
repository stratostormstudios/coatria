...
export const failInput=leaseInput.extend({clientId:uuid,error:z.string().trim().min(1).max(2000),retryable:z.boolean().optional().describe('False ends the run immediately. Omitted or true uses the server retry policy, including attempt and mission limits. Keep this disposition unchanged when replaying a clientId. Omitted and true are equivalent for legacy receipts.')}).strict();
...
export async function finishAgentRun(identity:AgentRunIdentity,runId:string,kind:'complete'|'fail',input:unknown){
 ...
 return transaction(async client=>{
  // Preserve pre-retryable receipt hashes; only a terminal failure adds a field.
  const access=await lockedRun(client,identity,runId),digest=hashToken(JSON.stringify({kind,...('result'in data?{result:data.result,artifactUrl:data.artifactUrl||null}:{error:data.error,...data.retryable===false?{retryable:false}:{}})}));
