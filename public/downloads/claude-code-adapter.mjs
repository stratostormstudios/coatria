...
export async function claudeInvocation({run,context,tools,mcpEnvironment},settings=process.env){
 ...
 if(!['api-key','native'].includes(authMode))throw new Error('Choose api-key or native Claude Code authentication.');
 if(authMode==='native'&&['ANTHROPIC_API_KEY','ANTHROPIC_AUTH_TOKEN','CLAUDE_CODE_OAUTH_TOKEN','ANTHROPIC_BASE_URL','ANTHROPIC_PROFILE','CLAUDE_CODE_USE_BEDROCK','CLAUDE_CODE_USE_VERTEX','CLAUDE_CODE_USE_FOUNDRY'].some(key=>settings[key]))throw new Error('Native login requires removing provider credential and endpoint overrides from this worker. Sign in using the official Claude Code CLI.');
 const config=providerConfiguration(context,settings,authMode==='native'?{nativeClaudeLogin:true}:undefined);if(config.provider!=='anthropic'||context.installation.pluginId!=='claude-code')throw new Error('Install the Claude Code bridge before using this adapter.');
...
export async function execute(options){
 ...
 return new Promise((resolveRun,reject)=>{
  ...
  child.on('error',()=>finish(new Error('Claude could not start. Verify the CLI path, version and selected worker authentication.')));
