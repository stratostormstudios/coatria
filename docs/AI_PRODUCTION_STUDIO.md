...
| Connect a bounded host | Register a company-scoped host, enroll exact installation revisions, and explicitly activate the chosen identities. A separately deployed supervisor retrieves encrypted credentials through a short lease. | Server keyring, privately configured host token, approved Runpod endpoint/model, and an operator-started CPU host. |
...
| Prepare delivery | Record an immutable package manifest containing approved exact versions, complete the delivery handoff task, and attest client acceptance of that exact package. | Current human approvals. Preparing a manifest does not transfer files or obtain a client's acknowledgement. |
...
A sole owner can set up the company and generate draft work with QC unassigned. Independent review remains blocked until a suitable second administrator joins. The person who produced or sponsored an output cannot independently approve it. Adding more agent characters does not satisfy that separation.
...
Studio can create a paused coordinator mission using the existing bounded mission engine. The human reviews and activates it; connected workers advance only their approved missions and assigned work. Cycles, cadence, run leases, task revisions, and per-run inference bounds are enforced. Agent messages and task context cannot grant permissions, approve work, change provider budgets, hire identities, or start paid hosts.

This is supervised autonomous execution within explicit permissions. It is not a general business operator that independently signs contracts, buys compute, hires a fleet, invoices clients, or makes final creative and commercial decisions. A long list of active missions is also not a company-wide dollar budget: provider spending reservations, shared fleet quotas, and invoice reconciliation remain separate work.
...
The renderer worker writes local files and its completion manifest. The separate [publisher CLI](VFX_EXECUTION.md#publish-completed-outputs-to-private-storage) uploads and verifies those exact completed outputs; invoking it is an explicit operator step, not part of the renderer polling loop. The integrated local browser/HTTP/Blender/private-Blob workflow passed all seven checks, including seven verified files, duplicate-free publication replay, independent review, and an exact delivery manifest. It used a synthetic company, ran no inference, and sent no client delivery. Signed storage links are bearer grants until they expire, so removing a company member cannot recall an already issued URL immediately. Keep the grants short and avoid persisting or logging them.
...
The remaining production gates include automatic host provisioning/reaping, immutable reviewed worker images, cross-host recovery, private-media transport automation, heavy-file resumable transfer, NAS connectivity, client review/receipt workflows, company-wide spending reservations, and measured multi-company concurrency. The current pilot has explicit limits; it does not establish million-user capacity or fully unattended end-to-end business operation.
