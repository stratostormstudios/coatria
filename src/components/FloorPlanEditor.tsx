...
import {ArrowLeft,ArrowUpRight,Check,Copy,Grip,Hand,Maximize,Minus,MousePointer2,Plus,Redo2,RotateCw,Save,Scan,Trash2,Undo2,AlertCircle} from 'lucide-react';
...
import {clamp,editorPlan,floorMaximum,floorMinimum,furniture,metres,moveItem,newItem,place,resizeFloor,resizeItem,rotateItem,round,signature,snap,type EditorPlan,type Edge,type Rect} from '@/lib/floor-editor';
...
type Gesture={kind:'move'|'resize'|'floor'|'palette'|'pan';pointerId:number;startX:number;startY:number;before:EditorPlan;scale:number;id?:string;edge?:Edge;type?:LayoutItem['type'];moved:boolean;scrollLeft:number;scrollTop:number;capture:HTMLElement};
...
export default function FloorPlanEditor(p:WorkspaceProps&{onDone:()=>void}) {
  ...
  function add(type:LayoutItem['type'],point?:{x:number;y:number}){if(current.current.layout.length>=MAX_LAYOUT_ITEMS)return;const item=newItem(type,current.current,crypto.randomUUID(),point,snapping);commit({...current.current,layout:[...current.current.layout,item]});setSelected(item.id);setAnnouncement(item.label+' added. Drag to move or use the dimension fields.');}
  ...
  function updateRect(key:keyof Rect,value:number){if(!active||!activeRect)return;const item=place(active,{...metres(active,current.current.floor),[key]:value},current.current.floor);commit(replaceItem(item));}
  ...
  function finish(cancel=false){const g=gesture.current;if(!g)return;gesture.current=null;
    ...
    else if(!cancel&&g.kind==='palette'){if(g.moved&&ghostRef.current){const item={...ghostRef.current,id:crypto.randomUUID()};commit({...current.current,layout:[...current.current.layout,item]});setSelected(item.id);setAnnouncement(item.label+' placed.');}else if(!g.moved&&g.type)add(g.type);}
  ...
  function start(event:ReactPointerEvent<HTMLElement>,kind:Gesture['kind'],extra:Partial<Gesture>={}){
    ...
    gesture.current={kind,pointerId:event.pointerId,startX:event.clientX,startY:event.clientY,before:current.current,scale,id:extra.id,edge:extra.edge,type:extra.type,moved:false,scrollLeft:viewport.current?.scrollLeft||0,scrollTop:viewport.current?.scrollTop||0,capture:event.currentTarget};
    setLockedScale(scale);setLockedOrigin({left:board.current?.offsetLeft||50,top:board.current?.offsetTop||50});setGestureKind(kind);if(extra.id)setSelected(extra.id);
  ...
  handlers.current={finish,undo,redo,remove,duplicate,rotate,move(event){const g=gesture.current;if(!g||event.pointerId!==g.pointerId)return;const dx=(event.clientX-g.startX)/g.scale,dy=(event.clientY-g.startY)/g.scale;if(Math.hypot(event.clientX-g.startX,event.clientY-g.startY)>3)g.moved=true;if(!g.moved)return;
    ...
    if(g.kind==='palette'){const bounds=board.current?.getBoundingClientRect(),inside=bounds&&event.clientX>=bounds.left&&event.clientX<=bounds.right&&event.clientY>=bounds.top&&event.clientY<=bounds.bottom;const item=inside&&g.type?newItem(g.type,g.before,'drag-preview',{x:(event.clientX-bounds.left)/g.scale,y:(event.clientY-bounds.top)/g.scale},grid):null;if(item)item.id='new-'+g.pointerId;ghostRef.current=item;setGhost(item);return;}
  ...
  function keys(event:React.KeyboardEvent){if(editable(event.target))return;if(event.key==='Escape'){event.preventDefault();if(gesture.current)finish(true);else setSelected(null);return;}
  ...
  return <div ref={root} className={s.editor} onKeyDown={keys} onKeyUp={event=>{if(event.code==='Space')spaceHeld.current=false;}}>
    ...
    <div className={s.studio}>
      <aside className={s.palette} aria-label="Furniture library"><header><span className="eyebrow">BUILDING BLOCKS</span><h2>Make room for work.</h2><p>Drag onto the floor, or click to add.</p></header>
        <div className={s.library}>{furniture.map(item=><button key={item.type} aria-label={item.name} disabled={plan.layout.length>=MAX_LAYOUT_ITEMS} className={s.paletteItem} onPointerDown={event=>start(event,'palette',{type:item.type})} onClick={event=>{if(event.detail===0)add(item.type);}}><span className={s.paletteGlyph}><FurnitureGlyph type={item.type}/></span><span><strong>{item.name}</strong><small>{item.description}</small></span><Grip size={14}/></button>)}</div>
        <div className={s.libraryNote}><span>{plan.layout.length} / {MAX_LAYOUT_ITEMS} objects</span><p>Visual spaces shape your office. Create their conversations separately in Rooms.</p></div>
      </aside>
      ...
      <div ref={viewport} data-testid="floor-viewport" className={s.viewport+' '+(panMode||gestureKind==='pan'?s.panning:'')} onPointerDown={event=>{if(panMode||spaceHeld.current||event.button===1)start(event,'pan');}}>
        <div className={s.stage} style={stageStyle}>
          <div ref={board} data-testid="floor-plan" aria-label="Office floor plan" aria-describedby="floor-keyboard-help" tabIndex={0} className={s.floor} style={boardStyle} onPointerDown={event=>{if(panMode||spaceHeld.current||event.button===1)start(event,'pan');else if(event.target===event.currentTarget){setSelected(null);event.currentTarget.focus({preventScroll:true});}}}>
            ...
            {plan.layout.map(item=><div key={item.id} data-object-id={item.id} data-object-type={item.type} className={s.object+' '+s[item.type]+' '+(selected===item.id?s.selected:'')} style={{left:item.x+'%',top:item.y+'%',width:item.w+'%',height:item.h+'%',zIndex:selected===item.id?3:1}}>
              <button className={s.objectBody} aria-label={'Select '+item.label} aria-pressed={selected===item.id} title={item.label} onPointerDown={event=>start(event,'move',{id:item.id})} onClick={()=>setSelected(item.id)}><FurnitureGlyph type={item.type} rotation={item.rotation}/><span>{item.label}</span></button>
            ...
            {ghost&&<div className={s.ghost} style={{left:ghost.x+'%',top:ghost.y+'%',width:ghost.w+'%',height:ghost.h+'%'}}><FurnitureGlyph type={ghost.type}/><span>Drop here</span></div>}
      ...
      <aside className={s.inspector} aria-label="Floor plan properties">
        ...
        <section><header><span className="eyebrow">{active?'YOUR SELECTION':'OBJECT PROPERTIES'}</span><h2>{active?furniture.find(item=>item.type===active.type)?.name:'Select a building block'}</h2></header>
          {active&&activeRect?<><Field label="Label"><input value={active.label} maxLength={80} onChange={event=>{if(!labelStart.current)labelStart.current=current.current;draft(replaceItem({...active,label:event.target.value}));}} onBlur={()=>{if(labelStart.current)remember(labelStart.current,current.current);labelStart.current=null;}}/></Field>
            <div className={s.fields}><NumberField label="Left (m)" value={activeRect.x} max={plan.floor.width-activeRect.w} onValue={value=>updateRect('x',value)}/><NumberField label="Top (m)" value={activeRect.y} max={plan.floor.depth-activeRect.h} onValue={value=>updateRect('y',value)}/><NumberField label="Width (m)" value={activeRect.w} min={.5} max={plan.floor.width-activeRect.x} onValue={value=>updateRect('w',value)}/><NumberField label="Depth (m)" value={activeRect.h} min={.5} max={plan.floor.depth-activeRect.y} onValue={value=>updateRect('h',value)}/></div>
