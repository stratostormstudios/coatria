...
export async function rateLimit(key: string, limit: number, seconds: number) {
  const result = await query<{ count: number }>(`INSERT INTO rate_limits(key,count,expires_at) VALUES($1,1,now()+$2*interval '1 second')
    ON CONFLICT(key) DO UPDATE SET count=CASE WHEN rate_limits.expires_at < now() THEN 1 ELSE rate_limits.count+1 END,
    expires_at=CASE WHEN rate_limits.expires_at < now() THEN now()+$2*interval '1 second' ELSE rate_limits.expires_at END RETURNING count`, [hashToken(key), seconds]);
  if (result.rows[0].count > limit) fail(429, 'Too many requests. Try again later.', 'RATE_LIMITED');
}
