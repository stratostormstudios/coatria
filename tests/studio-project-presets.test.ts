...
test('explicit procedural preset fits actual project and controlled execution contracts without approving work',()=>{
 ...
 const project=studioProjectInput.parse({clientId:randomUUID(),name:'Controlled pilot',clientName:'Internal',brief:'Original procedural turntable only.',aiPolicy:'unknown',...preset});assert.equal(project.aiPolicy,'unknown');assert.deepEqual(project.shots[0].disciplines,['lighting']);assert.equal(project.shots[0].handles,0);
