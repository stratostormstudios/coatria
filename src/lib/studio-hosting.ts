...
export async function revokeStudioHost(client:PoolClient,member:Membership,hostId:string,input:unknown){
 ...
 const invalidatedAgentCount=await invalidateHostCredentials(client,member.companyId,hostId,'Managed host revoked by an administrator.');
 const result={host:await hostProjection(client,member.companyId,hostId),invalidatedAgentCount};await recordRequest(client,member,hostId,'revoke',data.clientId,hash,result);await client.query("INSERT INTO activity(company_id,actor_id,kind,description) VALUES($1,$2,'studio.host_revoked','A managed host was revoked; matching issued agent credentials and active requests were invalidated.')",[member.companyId,member.userId]);return {...result,replayed:false};
}
async function invalidateHostCredentials(client:PoolClient,companyId:string,hostId:string,reason:string){
 const current=(await client.query('SELECT c.agent_id,c.installation_id FROM studio_host_credentials c JOIN agents a ON a.company_id=c.company_id AND a.id=c.agent_id AND a.token_hash=c.token_hash WHERE c.company_id=$1 AND c.host_id=$2 AND c.revoked_at IS NULL FOR UPDATE OF c',[companyId,hostId])).rows;
 for(const c of current){await client.query("UPDATE agents SET token_hash=$3,status='paused',expires_at=LEAST(expires_at,clock_timestamp()) WHERE company_id=$1 AND id=$2",[companyId,c.agent_id,hashToken(secret('ca_'))]);await client.query('UPDATE plugin_installations SET revision=revision+1,updated_at=clock_timestamp() WHERE company_id=$1 AND id=$2',[companyId,c.installation_id]);}
 await cancelRuns(client,companyId,current.map(c=>c.agent_id),reason);await client.query('UPDATE studio_host_credentials SET revoked_at=clock_timestamp() WHERE company_id=$1 AND host_id=$2 AND revoked_at IS NULL',[companyId,hostId]);
 await client.query("UPDATE studio_managed_hosts SET status='revoked',revision=revision+1,revoked_at=COALESCE(revoked_at,clock_timestamp()),lease_epoch=lease_epoch+1,lease_owner=NULL,lease_expires_at=clock_timestamp() WHERE company_id=$1 AND id=$2 AND status<>'revoked'",[companyId,hostId]);
 return current.length;
}
/** Internal cleanup only. The caller holds the company CPU lock and commits the
 * terminal provision state in this same transaction. No current sponsor or
 * runtime configuration is required to remove already-issued authority. */
export async function revokeUnsubmittedStudioHostProvisionCredentials(client:PoolClient,companyId:string,provisionId:string){
 const provision=(await client.query("SELECT host_id,created_by FROM studio_host_provisions WHERE company_id=$1 AND id=$2 AND phase='failed' AND stop_requested_at IS NOT NULL AND submitted_at IS NULL AND pod_id IS NULL FOR UPDATE",[companyId,provisionId])).rows[0];
 if(!provision?.host_id)fail(409,'Only an unsubmitted failed provision with a stop request can revoke its enrollment.','CPU_CLEANUP_CONFLICT');
 await lockHostActors(client,companyId,provision.host_id);await hostRow(client,companyId,provision.host_id);
 const invalidatedAgentCount=await invalidateHostCredentials(client,companyId,provision.host_id,'Managed CPU provisioning failed before submission; cleanup revoked its enrollment.');
 await client.query("INSERT INTO activity(company_id,actor_id,kind,description) VALUES($1,$2,'studio.host_failed_cleanup',$3)",[companyId,provision.created_by,`Failed CPU provision ${provisionId} was closed before provider submission; its host and matching issued credentials were revoked. The compute reservation was retained.`]);
 return {invalidatedAgentCount};
}
