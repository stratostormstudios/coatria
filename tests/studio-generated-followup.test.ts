...
test('generated continuation executes exact leased tools against immutable synthetic verified sources',{skip:!emulate&&!localPostgres,timeout:180000},async t=>{
 ...
 try{
  ...
  async function fixture(kind:keyof typeof specs='image',options:{budget?:number;optIn?:boolean;reuse?:boolean}={}){
   ...
   const agents:Row[]=[];for(const name of['coordinator','specialist']){const agent=await insert('agents',{company_id:company,name,harness:'custom',created_by:producer.id,token_hash:sha(randomUUID()),capabilities:JSON.stringify(caps),invocation_access:'admins',expires_at:await expiry()});agents.push(agent);await insert('plugin_installations',{company_id:company,agent_id:agent.id,installed_by:producer.id,client_id:randomUUID(),request_hash:sha(randomUUID()),plugin_id:'runpod',manifest_version:'1.0.0',runtime_config:JSON.stringify({providerId:'runpod',modelId:'synthetic-only'}),character:'{}'});}
   const[coordinator,specialist]=agents;
   await transaction(c=>setupStudio(c,{companyId:company,userId:producer.id},{clientId:randomUUID(),revision:0,templateId:'ai-production',templateVersion:1,assignments:[{roleKey:'coordinator',agentId:coordinator.id},{roleKey:'comp',agentId:specialist.id}]}));
  ...
  for(const kind of['image','video','audio'] as const)await t.test(kind+' claims, registers and submits exactly once even with new outer transport IDs',async()=>{
   const f=await fixture(kind),queued=(await f.dispatch()).result as Row;assert(queued.childRunId);assert.notEqual(queued.childRunId,f.source.run.id);assert.equal((await f.policy()).runs_started,2);assert.equal(((await f.dispatch()).result as Row).childRunId,queued.childRunId);
