...
function PolicyEditor({p,detail,initial,path,onClose,onSaved}:{p:WorkspaceProps;detail:StudioReadableProjectDetail;initial:Policy|null;path:string;onClose:()=>void;onSaved:()=>Promise<void>}){
 ...
 async function submit(event:FormEvent){event.preventDefault();await mutation.mutate(path,{revision:initial?.revision||0,coordinatorAgentId,allowedRoleKeys,maxRuns,maxConcurrentRuns,status,expiresAt},onSaved,'PUT');}
 return <Modal title="Review project delegation" description="Give one coordinator a bounded ability to request work from reviewed specialists." onClose={()=>{if(!mutation.busy)onClose();}} wide><form className="form" onSubmit={submit}><fieldset disabled={mutation.busy}>
  ...
  <label className={s.checkbox}><input required type="checkbox" checked={reviewed} onChange={event=>setReviewed(event.target.checked)}/>I reviewed the coordinator, selected roles, request allowance, concurrency, state, and exact expiry.</label>
