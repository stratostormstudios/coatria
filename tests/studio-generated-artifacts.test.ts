...
test('generated registration authenticates verified synthetic storage and preserves original attribution',{skip:!emulate&&!localPostgres,timeout:180000},async t=>{
 ...
 try{
  ...
  // Exercise current service authority against the complete schema. Migration
  // cutoff/upgrade compatibility is isolated in the persistence fixture.
  for(const file of(await readdir('database')).filter(f=>/^\d.*\.sql$/.test(f)).sort())await db.query(await readFile('database/'+file,'utf8'));
  ...
  async function fixture(kind:keyof typeof specs='image',options:{media?:Row;missingVerification?:boolean;agent?:boolean;shared?:Row}={}){
   ...
   if(!shared){const qcTask=await insert(db,'tasks',{company_id:company,title:'Synthetic independent QC',created_by:producer}),qc=await insert(db,'studio_work_items',{company_id:company,project_id:p.id,shot_id:u.id,logical_key:'D010:qc',task_id:qcTask.id,stage:'qc',role_key:'qc',execution:'human'});await insert(db,'studio_dependencies',{company_id:company,project_id:p.id,work_item_id:qc.id,predecessor_id:w.id});}
  ...
  await t.test('100 historical artifacts use one source query and constant project queries with full validation',{timeout:60000},async t=>{
   ...
   // Current-round identity, baseline work and unit scope add three bounded
   // metadata reads; historical source validation remains one batched query.
   ...
   t.diagnostic(`Synthetic history 1/100 artifacts: ${single.elapsedMs.toFixed(1)}/${batch.elapsedMs.toFixed(1)} ms, 1/1 queries; project detail: ${smallProject.elapsedMs.toFixed(1)}/${largeProject.elapsedMs.toFixed(1)} ms, 13/13 queries.`);
   ...
   const history=await countQueries(c=>studioProjectDetail(c,f.company,f.p.id,2));assert.deepEqual(history.result.artifacts,largeProject.result.artifacts);assert.equal(history.statements.length,13);await rejects(c=>loadStoredGeneratedArtifact(c,f.company,f.p.id,first.artifact.id,{requireAvailable:true}),'STUDIO_GENERATED_EVIDENCE_INVALID');
