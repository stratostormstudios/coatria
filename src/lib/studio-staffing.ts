...
export async function proposeStudioStaffing(client:PoolClient,actor:StudioActor,input:unknown){
 ...
 if(specialists.length<data.teamSize)warnings.push(`The requested ${data.teamSize} agents exceed the ${specialists.length} useful role groups in this scope; only ${specialists.length} identities are proposed.`);
 ...
 const plan:StudioStaffingPlan={version:1,templateId:'vfx-boutique',templateVersion:1,brief:data.brief,requestedAgentCount:data.teamSize,actualAgentCount:specialists.length,newAgentCount:specialists.filter(s=>s.mode==='create').length,disciplines:[...data.disciplines],reviewer:data.reviewerHumanId&&reviewer?{humanId:data.reviewerHumanId,...reviewer}:null,profileRevision:profile?.revision??0,specialists,unassignedRoleKeys:STUDIO_TEMPLATES[0].roles.filter(role=>role.key!=='qc'&&!draft.requiredRoleKeys.includes(role.key)).map(role=>role.key),warnings,startsWorkers:false,startsInference:false,copiesPrivateSkills:false,newIdentityStatus:'paused',credentialDelivery:'not_issued'};
 ...
 await client.query("INSERT INTO activity(company_id,actor_id,kind,description) VALUES($1,$2,'studio.staffing_proposed',$3)",[actor.companyId,actor.userId,`A ${specialists.length}-identity staffing plan was proposed for human review; no workers or credentials were issued.`]);
...
export async function applyStudioStaffing(client:PoolClient,member:Membership,proposalId:string,input:unknown){
 ...
 for(const specialist of plan.specialists){
  ...
  const agent=(await client.query("INSERT INTO agents(company_id,name,harness,description,token_hash,created_by,status,conversation_access,invocation_access,capabilities,expires_at) VALUES($1,$2,$3,$4,$5,$6,'paused','none','admins',$7,clock_timestamp()+interval '30 days') RETURNING id",[member.companyId,installation.name,entry.harness,`Studio specialist: ${specialist.roleKeys.join(', ')}. Shared curated skills; requires worker enrollment.`,tokenHash,member.userId,JSON.stringify(installation.capabilities)])).rows[0];
 ...
 const result:StudioStaffingApplication={proposalId,profileRevision:configured.profile!.revision,specialists:applied,reviewerHumanId:plan.reviewer?.humanId??null,startsWorkers:false,startsInference:false,credentialDelivery:'not_issued'};
