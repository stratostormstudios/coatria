...
The Runpod pilot endpoint remains configured with zero allowed workers after verified shutdown. Connecting it to a real company requires an authorized installation and an online operator-managed worker. The remaining providers have protocol and failure-path fixture coverage. Live model access, billing and tool behavior still require account-specific canaries. No provider key is entered in the browser or stored in Coatria's database by this release. See [the measured Runpod setup and limitations](RUNPOD_INFERENCE.md).
