...
- POST /api/companies/:id/agents `{name,harness:'hermes'|'custom'|'claude-code'|'codex',description,conversationAccess?:'none'|'read'|'write',invocationAccess?:'none'|'members'|'admins',capabilities?:AgentCapability[],expiresInDays?:number}` -> `{agent,token}` owner/admin only. Agent `{id,name,harness,description,status,createdBy,lastSeenAt,conversationAccess,invocationAccess,capabilities,expiresAt,presence}`; token shown once and stored as a hash. Both access modes default to `none`, capabilities to `[]`, expiry to 90 days (allowed 1–365). The nullable `presence` projection contains only this bot's recent position/status, never a lease or credential.
- PATCH /api/companies/:id/agents/:agentId `{status?:'active'|'paused'|'revoked',conversationAccess?:'none'|'read'|'write',invocationAccess?:'none'|'members'|'admins',capabilities?:AgentCapability[]}` -> `{agent}` owner/admin only; at least one change is required. A revoked credential cannot be reactivated. Permission changes are rechecked on subsequent requests; a request already holding authority may complete before the revocation transaction commits. Ending invocation authority cancels affected queued/running requests; capability reductions are checked against each run's original capability snapshot.
- POST /api/companies/:id/agents/:agentId/rotate `{expiresInDays?:number}` -> `{agent,token}` owner/admin only; 1–365 days, default 90. Invalidates the old credential and cancels queued/running requests. Revoked agents must be registered anew.
- Legacy `GET /api/agent/work` and `POST /api/agent/report` return **HTTP 410**. Use explicit durable invocations and leased tools; the retired downloaded client performs no network requests.

The [runtime guide](../public/downloads/AGENT_RUNTIME.md) covers the [Codex adapter](../public/downloads/codex-adapter.mjs), [external worker](../public/downloads/agent-worker.mjs), [MCP stdio bridge](../public/downloads/agent-mcp.mjs), private state and restart behavior. The [runtime OpenAPI contract](https://coatria.com/api/agent/openapi) derives tool request schemas from the server definitions. Selecting a harness in the UI does not install or start that provider.

| Surface | Contract |
|---|---|
| Browser invoke | `POST /api/companies/:id/conversations/:channel/runs` with `{clientId:UUID,agentId:UUID,prompt,parentId?:UUID|null}`; prompt 1–6,000 characters; returns `{run,replayed}` (201 new, 200 replay) |
| Browser list/detail | `GET /api/companies/:id/agent-runs` or `.../conversations/:channel/runs`; optional `agentId`, `parentId`, `limit` 1–50; detail `.../agent-runs/:runId` returns `{run,actions}` where actions are committed tool name/request-ID/time receipts |
| Browser cancel | `POST .../agent-runs/:runId/cancel` with `{}`; requester or current owner/admin; returns `{run}` |
| Claim | `POST /api/agent/runs/claim` with `{workerId,claimId:UUID}`; returns `{run:null,replayed}` or `{run,leaseToken,leaseExpiresAt,replayed}` |
| Renew/context | `POST /api/agent/runs/:runId/heartbeat` with `{leaseToken}`; `GET .../:runId/context` with `X-Coatria-Run-Lease`; context returns `{run,messages,capabilities}` for at most 30 source conversation/thread messages |
| Tools | `GET /api/agent/tools` returns current-grant metadata. `POST .../tools/:name` with `{runId,leaseToken,requestId:UUID,arguments}` returns `{result,replayed}`; lease belongs in JSON for execution |
| Complete/fail | `POST /api/agent/runs/:runId/complete` with `{leaseToken,clientId:UUID,result,artifactUrl?}` (result 1–12,000); `/fail` with `{leaseToken,clientId:UUID,error}` (1–2,000); both return `{run,replayed}` |
| Human proposal review | Owner/admin `GET /api/companies/:id/agent-proposals` with optional `runId`, `after`, `limit`; `POST .../agent-proposals/:proposalId/approve` or `/reject` with `{}`. Approval checks current authority, exact stored data and floor revision; hiring approval creates an unpublished draft |

Run statuses are `queued`, `running`, `succeeded`, `failed`, `cancelled`. Claims use 60-second leases, normally renewed every 15 seconds, and at most three attempts within 30 minutes of first start. A claim ID retries an uncertain claim; after a confirmed idle response, use a new ID. The same mutation request ID deduplicates the same tool/run/arguments. Completion receipts deduplicate an accepted result and its atomic agent-authored conversation reply. External model/file/repository effects still require their own idempotency or reconciliation. Stale leases return `409 RUN_LEASE_LOST`; cancellation returns `409 RUN_CANCELLED`; access loss returns `401/403`. Backoff on `429 Retry-After`.

Capabilities are `workspace.read`, `tasks.write`, `infrastructure.read`, `hiring.read`, `office.write`, `layout.propose`, `rooms.propose`, `hiring.propose`. All 18 default tool names and their boundaries are listed in the runtime guide. Tools never inherit all of a sponsor's admin powers: each execution checks current sponsor/requester membership, current agent grants, original run grants and a live lease. Task writes require requester authority and run ownership, and cannot approve work. Layout/room/hiring writes create reviewable proposals. No default tool reads personal vaults, original NAS footage, provider credentials, applicants or billing data.
