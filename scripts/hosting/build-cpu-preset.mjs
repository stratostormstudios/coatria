...
export async function buildCpuPreset({root,commit,configuration}){
 ...
 if(!configuration||typeof configuration!=='object'||Object.keys(configuration).some(key=>!keys.includes(key)&&key!=='inference')||keys.some(key=>!(key in configuration)))throw Error('Use only the documented nonsecret CPU preset configuration fields.');
 if(configuration.inference!==undefined){const v=configuration.inference,expected=['mode','maxJobs','maxHourlyMicrousd','lifetimeAllowanceMicrousd'];if(!v||typeof v!=='object'||Array.isArray(v)||Object.keys(v).length!==expected.length||expected.some(key=>!(key in v))||v.mode!=='coatria_broker_v1'||!Number.isSafeInteger(v.maxJobs)||v.maxJobs<1||v.maxJobs>100||!Number.isSafeInteger(v.maxHourlyMicrousd)||v.maxHourlyMicrousd<1000||v.maxHourlyMicrousd>10000000||!Number.isSafeInteger(v.lifetimeAllowanceMicrousd)||v.lifetimeAllowanceMicrousd<1||v.lifetimeAllowanceMicrousd>20000000)throw Error('Use only the exact documented nonsecret inference limits.');}
