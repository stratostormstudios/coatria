...
export const agentColumns = `id,name,harness,description,status,conversation_access AS "conversationAccess",invocation_access AS "invocationAccess",capabilities,expires_at AS "expiresAt",created_by AS "createdBy",last_seen_at AS "lastSeenAt",(SELECT json_build_object('x',p.x,'z',p.z,'roomId',p.room_id,'status',p.status,'updatedAt',p.updated_at) FROM agent_presence p WHERE p.company_id=agents.company_id AND p.agent_id=agents.id AND p.updated_at>clock_timestamp()-interval '60 seconds') AS presence`;
