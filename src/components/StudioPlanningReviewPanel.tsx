...
function ReviewEditor({p,detail,initial,path,onClose,onSaved}:Props&{initial:StudioReviewPolicy|null;path:string;onClose:()=>void;onSaved:()=>Promise<void>}){
 ...
 return <Modal title="Review the planning policy" description="Authorize a different agent to accept exact planning submissions within this project." onClose={()=>{if(!mutation.busy)onClose();}} wide><form className="form" onSubmit={submit}><fieldset disabled={mutation.busy}>
  ...
  <Field label="Separate planning reviewer" hint="Grant studio.read and studio.review to a dedicated plugin agent in Plugins first. Existing agents receive no new permissions automatically."><select required value={reviewerAgentId} onChange={event=>{setReviewer(event.target.value);changed();}}><option value="">Choose a reviewer with studio.review</option>{reviewers.map(agent=><option key={agent.id} value={agent.id}>{agent.name}</option>)}</select></Field>
  <fieldset className={s.disciplines}><legend>Planning stages this reviewer may accept</legend>{['estimate','breakdown','ingest'].map(stage=><label key={stage}><input type="checkbox" checked={allowedStages.includes(stage)} onChange={event=>{setStages(event.target.checked?[...allowedStages,stage]:allowedStages.filter(value=>value!==stage));changed();}}/>{stage==='estimate'?'Scope & estimate draft':stage==='breakdown'?'Shot breakdown & schedule':'Agent-assigned ingest provenance'}</label>)}</fieldset>
  <p className={s.muted}>Estimate review accepts the planning contribution; the commercial approval gate remains a human decision. Ingest review applies only to explicitly agent-assigned provenance work, never human media inspection.</p>
