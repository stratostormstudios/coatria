...
export function sealHiggsfieldSecret(value:unknown,context:Context):Envelope{
 const text=JSON.stringify(value);if(Buffer.byteLength(text)>(context.purpose==='provider-response'?262144:65536))fail(413,'Connection credentials exceed the vault limit.');
...
export function openHiggsfieldSecret<T>(value:Envelope,context:Context):T{
 ...
 try{const nonce=Buffer.from(value.nonce,'base64'),tag=Buffer.from(value.tag,'base64');if(nonce.length!==12||tag.length!==16||value.ciphertext.length>(context.purpose==='provider-response'?350000:90000))throw Error();const decipher=createDecipheriv('aes-256-gcm',key,nonce);decipher.setAAD(aad(context));decipher.setAuthTag(tag);return JSON.parse(Buffer.concat([decipher.update(Buffer.from(value.ciphertext,'base64')),decipher.final()]).toString('utf8')) as T;}
