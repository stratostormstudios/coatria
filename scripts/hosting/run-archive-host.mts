...
import {ArchiveHostRunError,archiveHostCanarySummary} from './archive-host-diagnostics.mjs';
...
async function delegated(mode:Mode,checkpoint:Checkpoint){
 checkpoint('delegation_path');
 const paths=archiveHostDelegatedPath(await readFile('/proc/self/cgroup','utf8'),mode);
 checkpoint('delegation_ownership');
 ...
 const limits={'memory.max':['2147483648','delegation_memory_limit'],'memory.swap.max':['0','delegation_swap_limit'],'pids.max':['256','delegation_process_limit'],'cpu.max':['200000 100000','delegation_cpu_limit']};for(const [name,[value,stage]]of Object.entries(limits)){checkpoint(stage);if(await text(join(paths.serviceRoot,name))!==value)archiveHostFailure();}
 checkpoint('delegation_empty_parent');
 ...
 checkpoint('delegation_controllers');
 ...
 checkpoint('delegation_enable');await writeFile(join(paths.serviceRoot,'cgroup.subtree_control'),'+cpu +memory +pids');checkpoint('delegation_create');try{await mkdir(paths.cgroupRoot,{mode:0o700});}catch(error){if((error as NodeJS.ErrnoException).code!=='EEXIST')throw error;}
 checkpoint('delegation_child_integrity');
 ...
 checkpoint('delegation_child_enable');await writeFile(join(paths.cgroupRoot,'cgroup.subtree_control'),'+cpu +memory +pids');return paths;
}
async function hostInput(bundleSha256:string,checkpoint:Checkpoint):Promise<Host>{
 checkpoint('host_platform');
 ...
 checkpoint('host_configuration');const path=join(ARCHIVE_HOST_CONFIG,'host.json');await archiveHostTrusted(path);const host=JSON.parse((await archiveHostRead(path)).toString('utf8')) as Host;
 ...
 checkpoint('bundle_integrity');const bundle=await inspectArchiveHostBundle(host.release,bundleSha256,{trusted:true});if(bundle.commit!==host.commit)archiveHostFailure();
 checkpoint('profile_integrity');
...
async function qualify(host:Host,paths:ReturnType<typeof archiveHostDelegatedPath>,checkpoint:Checkpoint){
 checkpoint('qualification_staging');
 ...
 const config:MediaSandboxCanaryConfig={version:1,...paths,profiles:host.profiles,uid:host.uid,gid:host.gid,hostCanaryPath,hostCanarySha256:archiveHostHash(canary),evidence:root,fixtureRoot,sourceRoot:host.release+'/app',diagnostics:true,diagnosticScope:'archive-host-qualification'};
 checkpoint('qualification_canary');try{await runMediaSandboxCanary(config);}catch(error){let summary=null;try{summary=archiveHostCanarySummary(JSON.parse((await archiveHostRead(join(root,'qualification.json'),1024**2)).toString('utf8')));}catch{/* Missing or malformed failure evidence cannot qualify the host. */}throw new ArchiveHostRunError('qualification_canary',error,summary);}
 checkpoint('qualification_result');const report=JSON.parse((await archiveHostRead(join(root,'qualification.json'))).toString('utf8'));
...
async function worker(host:Host,paths:ReturnType<typeof archiveHostDelegatedPath>,mode:'worker'|'preflight',checkpoint:Checkpoint){
 ...
 checkpoint('worker_enablement');if(mode==='worker')await archiveHostTrusted(join(ARCHIVE_HOST_CONFIG,'worker-enabled'));
 checkpoint('worker_environment');
 ...
 checkpoint('worker_scratch');const scratch=join(ARCHIVE_HOST_STATE,'scratch');try{await mkdir(scratch,{mode:0o700});}catch(error){if((error as NodeJS.ErrnoException).code!=='EEXIST')throw error;}
 ...
 checkpoint('worker_process');const child=spawn(process.execPath,['--import','tsx','scripts/higgsfield-archive-worker.ts',...mode==='preflight'?['--preflight']:[]],{shell:false,stdio:'inherit',cwd:host.release+'/app',env});const stop=(signal:NodeJS.Signals)=>child.kill(signal);const term=()=>stop('SIGTERM'),interrupt=()=>stop('SIGINT');process.once('SIGTERM',term);process.once('SIGINT',interrupt);try{const status=await new Promise<number>((resolve,reject)=>{child.once('error',reject);child.once('close',code=>resolve(code??1));});if(status!==0)archiveHostFailure();}finally{process.removeListener('SIGTERM',term);process.removeListener('SIGINT',interrupt);}
}
export async function runArchiveHost(mode:Mode,bundleSha256:string){let stage='arguments';const checkpoint:Checkpoint=value=>{stage=value;};try{if(!['qualify','preflight','worker'].includes(mode))archiveHostFailure();const host=await hostInput(bundleSha256,checkpoint),paths=await delegated(mode,checkpoint);if(mode==='qualify')await qualify(host,paths,checkpoint);else await worker(host,paths,mode,checkpoint);}catch(error){throw error instanceof ArchiveHostRunError?error:new ArchiveHostRunError(stage,error);}}
if(process.argv[1]&&import.meta.url===pathToFileURL(resolve(process.argv[1])).href){void(async()=>{const [mode,hash,...extra]=process.argv.slice(2);try{if(extra.length||!hash)archiveHostFailure();await runArchiveHost(mode as Mode,hash);}catch(error){console.error(JSON.stringify((error instanceof ArchiveHostRunError?error:new ArchiveHostRunError('arguments',error)).diagnostic));process.exitCode=1;}})();}
