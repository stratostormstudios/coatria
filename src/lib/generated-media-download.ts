...
export async function downloadGeneratedMedia(access:StorageDownloadAccess,artifact:StudioGeneratedArtifact,target:StorageSaveTarget|null,context:Context){
 ...
 try{
  context.signal?.throwIfAborted();assertGeneratedDownload(access,artifact,context.gatewayOrigin);
  const transport:typeof fetch=async(input,init)=>{
   ...
   if(declared!==null&&Number(declared)!==artifact.file.bytes){await response.body.cancel().catch(()=>{});throw Error('The stored file length differs from this registered version.');}
   const hash=new Sha256Js();let received=0;
   const stream=response.body.pipeThrough(new TransformStream<Uint8Array,Uint8Array>({
    transform(chunk,controller){context.signal?.throwIfAborted();received+=chunk.byteLength;if(received>artifact.file.bytes)throw Error('The download exceeded this registered file size.');hash.update(chunk);controller.enqueue(chunk);},
    async flush(){
     context.signal?.throwIfAborted();
     if(received!==artifact.file.bytes)throw Error('The download ended before the complete file arrived.');
     const actual=Array.from(await hash.digest(),byte=>byte.toString(16).padStart(2,'0')).join('');
     context.signal?.throwIfAborted();
     if(actual!==artifact.file.sha256)throw Error('The downloaded file checksum differs from this registered version. No file was saved.');
     verified=true;
    },
