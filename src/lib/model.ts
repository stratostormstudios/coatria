...
export const taskPatch = z.object({ expectedRevision: z.number({error:'Reload the task before saving. Its reviewed revision is required.'}).int().min(1).max(Number.MAX_SAFE_INTEGER-1), status: z.enum(['todo','doing','review','done']).optional(), title: text(160).optional(), description: z.string().trim().max(12000).optional(), assigneeId: uuid.nullable().optional(), submissionUrl: submissionUrl.nullable().optional(), reviewNote: z.string().trim().max(4000).optional() }).strict().refine(v => Object.keys(v).some(key=>key!=='expectedRevision'), 'Provide at least one change.');
...
export const taskColumns = `id,title,description,status,revision,assignee_id AS "assigneeId",created_by AS "createdBy",submission_url AS "submissionUrl",submission_summary AS "submissionSummary",review_note AS "reviewNote",submitted_by AS "submittedBy",submitted_agent_id AS "submittedAgentId",approved_by AS "approvedBy",approved_agent_id AS "approvedAgentId",machine_review_id AS "machineReviewId",created_at AS "createdAt",updated_at AS "updatedAt",(SELECT COALESCE(array_agg(DISTINCT author.user_id),'{}'::uuid[]) FROM (SELECT user_id FROM task_authors WHERE task_id=tasks.id UNION SELECT a.created_by AS user_id FROM contributions c JOIN agents a ON a.id=c.agent_id WHERE c.task_id=tasks.id) author) AS "authorIds"`;
export const agentColumns = `id,name,harness,description,status,(SELECT p.id FROM plugin_installations p WHERE p.company_id=agents.company_id AND p.agent_id=agents.id) AS "pluginInstallationId",conversation_access AS "conversationAccess",invocation_access AS "invocationAccess",capabilities,expires_at AS "expiresAt",created_by AS "createdBy",last_seen_at AS "lastSeenAt",(SELECT json_build_object('x',p.x,'z',p.z,'roomId',p.room_id,'status',p.status,'updatedAt',p.updated_at) FROM agent_presence p WHERE p.company_id=agents.company_id AND p.agent_id=agents.id AND p.updated_at>clock_timestamp()-interval '60 seconds') AS presence`;
