...
export async function loadVerifiedGeneratedSource(db:PoolClient,companyId:string,projectId:string,workItemId:string,archiveId:string,options:{requireAvailable?:boolean}={}){
 ...
 if(!a||requireAvailable&&(a.status!=='verified'||a.revoked_at)||!a.approved_by||!a.upload_id||!a.version_id)invalid();
 ...
 if(!row||requireAvailable&&(row.job_status!=='completed'||row.request_status!=='returned')||row.outcome!=='jobs'||row.kind!==row.job_kind||row.connection_id!==a.provider_connection_id||row.job_connection_id!==row.connection_id||row.receipt_connection_id!==row.connection_id||row.receipt_connection_revision!==row.connection_revision||row.receipt_request_hash!==row.request_hash||row.receipt_approved_by!==row.approved_by||row.work_item_id!==workItemId||row.locator_identity!==a.locator_identity)invalid();
 ...
 if(!equal(source,expected)||source.kind!==requirement.spec.kind||Boolean(source.agentId)!==Boolean(source.runId)||Boolean(source.agentId)!==Boolean(source.agentSponsorId))invalid();
 if(source.agentId){const run=(await db.query('SELECT agent_id,requested_by FROM agent_runs WHERE company_id=$1 AND id=$2',[companyId,source.runId])).rows[0];if(!run||run.agent_id!==source.agentId||run.requested_by!==source.requestedBy)invalid();}
