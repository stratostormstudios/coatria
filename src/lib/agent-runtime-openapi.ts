...
for(const[name,tool]of Object.entries(AGENT_TOOLS))paths['/api/agent/tools/'+name]={post:{
 ...
 ...((['studio.write','studio.execute'].includes(tool.capability)||name==='studio_staffing_get')?{'x-coatria-requester-roles':['owner','admin'],'x-coatria-approval-authority':false}:{}),
 description:'Live run lease required in the JSON body. Keep requestId stable for the same logical mutation across retries. Mutation receipts commit with effects; reads return the current view. Input schemas cannot express every authorization or cross-field constraint; server validation remains authoritative.'+((['studio.write','studio.execute'].includes(tool.capability)||name==='studio_staffing_get')?' The requester must remain a company owner or administrator. These tools create drafts or register declared artifact metadata; they never approve a plan, verify media, render, spend money or deliver files.':''),
}};
