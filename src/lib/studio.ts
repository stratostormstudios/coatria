...
async function recordReview(client:PoolClient,actor:StudioActor,projectId:string,input:unknown){
 const data=parse(studioReviewInput,input);return requestOnce(client,actor,data.clientId,'review:'+projectId,data,async()=>{
  ...
  if([a.produced_by,a.agent_sponsor_id].includes(actor.userId))fail(403,'An independent administrator who did not produce or sponsor this version must review it.','STUDIO_INDEPENDENT_REVIEW');
...
async function dispatchWork(client:PoolClient,member:Membership,projectId:string,input:unknown){
 ...
 return requestOnce(client,actor,data.clientId,'dispatch:'+projectId,data,async()=>{
  const preview=(await client.query('SELECT w.task_id,w.role_key,b.agent_id FROM studio_work_items w JOIN studio_role_bindings b ON b.company_id=w.company_id AND b.role_key=w.role_key WHERE w.company_id=$1 AND w.project_id=$2 AND w.id=$3',[member.companyId,projectId,data.workItemId])).rows[0];if(!preview)fail(404,'Studio work item not found.');if(!preview.agent_id)fail(409,'Assign and connect an agent to this role first.','STUDIO_AGENT_REQUIRED');
  const role=STUDIO_TEMPLATES[0].roles.find(r=>r.key===preview.role_key)!;
  const prompt=`Act as ${role.title} in this company. Read studio_get with projectId ${projectId} and workItemId ${data.workItemId} for current task ${preview.task_id}, role assignments, dependencies and the curated skills ${role.skills.join(', ')} before acting. Continue that existing task; do not create a duplicate. If all current permissions and gates allow, reserve it with tasks_claim using its current revision, prepare the requested production-planning contribution and submit with tasks_submit for independent human review. Do not approve work, render media, transfer files, contact clients, promise prices or deploy compute. Stop and report any missing input or authority. Return a short status report with the task ID and actual committed state.`;
  ...
  if(work.execution!=='agent')fail(409,'This step requires an artist, DCC connector or human review; the text harness cannot execute it.','STUDIO_EXTERNAL_EXECUTION_REQUIRED');
  if(work.readiness!=='ready')fail(409,work.blockedReason??'This work is already running or awaiting review.','STUDIO_WORK_BLOCKED');
  ...
  await activity(client,actor,'studio.work_dispatched','A ready production-planning task was queued for its assigned agent.');return {run:queued.run,project:await bump(client,member.companyId,projectId)};
 });
}
