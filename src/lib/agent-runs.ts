...
async function lockedRun(client:PoolClient,identity:AgentRunIdentity,runId:string){
 ...
 if(!run)fail(404,'Agent request not found.');await missionAuthority(client,identity.company_id,run,access.requesterRole);return{...access,run,capabilities:capabilities(run.capabilities).filter(capability=>capabilities(access.agent.capabilities).includes(capability))};
}
