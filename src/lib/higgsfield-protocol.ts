...
export const HIGGSFIELD_READ_TOOLS=['balance','models_list','models_get','models_search','models_recommend','estimate_image_cost','estimate_video_cost','jobs_wait','list_workspaces'] as const;
...
export const higgsfieldProposalInput=z.object({clientId:z.string().uuid(),projectId:z.string().uuid(),projectRevision:z.number().int().positive(),tool:z.enum(HIGGSFIELD_GENERATION_TOOLS),arguments:args,note:z.string().trim().min(1).max(2000)}).strict();
...
export type HiggsfieldRequest={id:string;projectId:string;tool:string;arguments:Record<string,unknown>;note:string;requestHash:string;status:'proposed'|'dispatching'|'returned'|'uncertain'|'rejected';createdAt:string;result?:unknown;errorCode?:string|null};
