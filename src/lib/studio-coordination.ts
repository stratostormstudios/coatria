...
export async function studioCoordinationSnapshot(client:PoolClient,companyId:string,projectId:string):Promise<StudioCoordinationSnapshot>{
 ...
 const dispatches=(await client.query('SELECT d.work_item_id AS "workItemId",d.parent_run_id AS "parentRunId",d.child_run_id AS "childRunId",d.coordinator_agent_id AS "coordinatorAgentId",d.specialist_agent_id AS "specialistAgentId",d.policy_revision AS "policyRevision",d.created_at AS "createdAt",r.status FROM studio_coordination_dispatches d JOIN agent_runs r ON r.company_id=d.company_id AND r.id=d.child_run_id WHERE d.company_id=$1 AND d.project_id=$2 ORDER BY d.created_at DESC,d.child_run_id DESC LIMIT 100',[companyId,projectId])).rows;
 return {policy,dispatches:dispatches.map(d=>({...d,createdAt:new Date(d.createdAt).toISOString()})) as any,budgetUnit:'specialist_runs',budgetScope:'coordinator_dispatched_runs_only',startsWorkers:false,startsInference:false};
}
...
export async function dispatchStudioWork(client:PoolClient,agent:Row,run:Row,input:unknown){
 ...
 const existing=(await client.query('SELECT child_run_id AS "childRunId",parent_run_id AS "parentRunId",work_item_id AS "workItemId" FROM studio_coordination_dispatches WHERE company_id=$1 AND project_id=$2 AND work_item_id=$3',[companyId,data.projectId,data.workItemId])).rows[0];
 ...
 const queued=await dispatchWork(client,member,data.projectId,{clientId:uuidFor(`coatria-coordination:${companyId}:${data.projectId}:${data.workItemId}`),revision:data.projectRevision,workItemId:data.workItemId});
...
export async function coordinationRunAuthority(client:PoolClient,companyId:string,run:Row):Promise<boolean>{
 ...
 const delegation=(await client.query('SELECT project_id,policy_revision FROM studio_coordination_dispatches WHERE company_id=$1 AND child_run_id=$2',[companyId,run.id])).rows[0];if(!delegation)return true;
 ...
 const active=await status(client,companyId,policy);return ['active','exhausted'].includes(active.effectiveStatus);
}
