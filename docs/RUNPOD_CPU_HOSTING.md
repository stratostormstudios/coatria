...
Execution failures are recorded with `retryable: false`; a generic adapter exception does not prove that replay is safe. A lost failure acknowledgement reuses the saved receipt and never executes the adapter again. The agent API still supports an explicit retryable failure from other trusted clients, but this hosted worker does not opt into it. Known broker failures retain an allowlisted code such as `INFERENCE_OUTPUT_INVALID`; unknown exceptions become `ADAPTER_FAILED` without copying exception text. Host logs include only the fixed event/code, validated run UUID, timestamp and known disposition. They do not contain raw provider output or private exception diagnostics. Inspect the run actions, broker status and existing provider job during reconciliation; do not export worker journals, which contain lease credentials and task context. Applying a worker update requires a newly reviewed immutable source/bootstrap; an existing pinned Pod does not automatically acquire these changes.
