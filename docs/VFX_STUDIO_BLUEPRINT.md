...
The `#studio` interface configures an 11-role VFX template, assigns existing people or agents, captures client briefs and delivery specifications, and displays shots, dependencies, task submissions, version reviews and delivery manifests. Setup creates role assignments and curated company skill instructions; it does not create agent identities, grant permissions, copy private skill vaults or start paid workers. Team-size input in the read-only blueprint is descriptive; it does not generate a customized staffing plan.

The backend reuses company authority, leased agent runs and independent task acceptance. It creates a bounded, deterministic workflow using the selected disciplines; it is not an editable dependency-graph engine. Current human API routes are:

- `GET /api/companies/{companyId}/studio` and `POST .../studio/setup`.
- `POST .../studio/projects`; `GET` and `PATCH .../studio/projects/{projectId}`.
- `POST .../studio/projects/{projectId}/dispatch`, `/gates`, `/artifacts`, `/reviews` and `/deliveries`.

OpenAPI 1.2.0 documents these routes and **23 leased agent tools**, including five studio tools: `studio_templates`, `studio_company_plan`, `studio_get`, `studio_plan` and `studio_artifact_register`. The new `studio.read` and `studio.write` grants require explicit approval. Agent writes require a current administrator requester; agents cannot record studio approvals, provision staff, change budgets or prepare deliveries. Project context is paginated and bounded. `studio_get` with `projectId` and `workItemId` retrieves assigned work and current dependency summaries directly; `projectId` and `artifactId` retrieves one complete artifact reference.

Migration [012_studio.sql](../database/012_studio.sql) adds 12 tables: profiles, role bindings, projects, shots, work items, dependencies, artifact references, reviews, gate events, delivery manifests, request receipts and dispatch records. These are the implemented schema; the expanded entity table later in this document is a proposal. [Runtime permissions](../database/runtime-permissions.sql) must be applied separately after migration.

An administrator can dispatch ready scope/estimate or shot-breakdown work to its assigned, explicitly permissioned agent. Existing approved missions can also inspect and advance that agent's assigned tasks. A running, connected worker is required. Independent acceptance opens downstream dependencies. Artifact registration stores immutable **declared metadata and an external reference**; it does not fetch media or verify a checksum. Delivery preparation records exact approved versions without transferring files. Client acceptance identifies one exact delivery manifest and is an administrator attestation, not a client portal event.

**Hosted-worker upgrade required:** the stopped pilot host is pinned to `77a77ebb55e376a9bee09c55db61dd4678342d8b`. That provider adapter rejects JSON Schema `format: date`, now present in the studio tool catalog. Before granting studio scopes or resuming studio work, the operator must pin the bootstrap to the new reviewed release commit and its verified source hashes, preserve the existing private worker state, and establish a fresh bounded hosting window. Deploying the website does not upgrade a pinned worker. This document does not claim that the host has been upgraded or that a live Qwen studio task has run.

## Still proposed or requires a connected service

Automatic staffing/worker provisioning, delegation between agents, company-wide spending reservations, editable graph branches, sequence/asset tracking, editorial conform/retime and downstream version invalidation remain future work. The current infrastructure connector indexes metadata; it does not mount a NAS, serve footage or perform resumable transfers. Nuke, Blender, Houdini, rendering, proxy generation, media/checksum inspection, annotated client review and delivery transport need separately implemented and tested connectors. Current Runpod inference is a language-model service. The tests prove authority and workflow behavior on synthetic records; they do not certify rendered output, client delivery or million-user capacity.
