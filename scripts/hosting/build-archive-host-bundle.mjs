...
export async function buildArchiveHostBundle({sourceRoot,commit,runtimeRoot,runtimeManifestSha256,output}){
 ...
 await verifyArchiveTree(runtimePath,runtime.files,{extra:['runtime-manifest.json'],emptyDirectories:runtime.emptyDirectories});
 ...
 await mkdir(join(target,'runtime'),{mode:0o755});await copyArchiveFiles(runtimePath,join(target,'runtime'),runtimeOnly,{emptyDirectories:runtime.emptyDirectories});await copyArchiveFiles(runtimePath,join(target,'app'),modules);files.push(...runtime.files.map(f=>({...f,path:archiveRuntimeTarget(f.path)})));files.sort((a,b)=>a.path.localeCompare(b.path,'en'));
 const directories=runtime.emptyDirectories.map(entry=>({...entry,path:archiveRuntimeTarget(entry.path)})),manifest=parseArchiveBundle({version:1,kind:'coatria-archive-host',commit,tree,runtimeManifestSha256:archiveHostHash(JSON.stringify(runtime)+'\n'),runtime,files,emptyDirectories:directories});const manifestBytes=JSON.stringify(manifest,null,2)+'\n';await writeFile(join(target,'bundle.json'),manifestBytes,{flag:'wx',mode:0o444});
 await verifyArchiveTree(target,files,{extra:['bundle.json'],emptyDirectories:directories});return {commit,tree,bundleSha256:archiveHostHash(manifestBytes),runtimeInputSha256:runtimeManifestSha256,output:target,qualified:false,servicesEnabled:false,providerCalls:false};
}
