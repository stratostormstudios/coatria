...
import {buildStudioGeneratedArtifactManifest,loadStoredGeneratedArtifact,loadVerifiedGeneratedSource,registerStudioGeneratedArtifact,studioGeneratedSourceInput} from '../src/lib/studio-generated-artifacts';
...
test('generated registration authenticates verified synthetic storage and preserves original attribution',{skip:!emulate&&!localPostgres,timeout:180000},async t=>{
 ...
 try{
  ...
  async function fixture(kind:keyof typeof specs='image',options:{media?:Row;missingVerification?:boolean;agent?:boolean}={}){
   const company=randomUUID(),producer=randomUUID(),registrar=randomUUID(),reviewer=randomUUID();for(const user of[producer,registrar,reviewer])await insert(db,'users',{id:user,name:'Synthetic registration fixture',email:user+'@example.invalid',password_hash:'not-a-login'});
   ...
   const p=await insert(db,'studio_projects',{company_id:company,name:'Generated '+kind,client_name:'Internal',brief:'Synthetic verified DB facts only; no provider calls',spec:JSON.stringify(spec),gates:JSON.stringify(gates),status:'production',ai_policy:'allowed',created_by:producer,production_path:'higgsfield',contract_version:2});
   const u=await insert(db,'studio_shots',{company_id:company,project_id:p.id,media_kind:kind,code:'D010',description:'Synthetic deliverable 🎬',frame_start:null,frame_end:null,handles:null,disciplines:'[]',duration_min_ms:kind==='image'?null:999,duration_max_ms:kind==='image'?null:1001});
   const task=await insert(db,'tasks',{company_id:company,title:'Synthetic generation',created_by:producer}),w=await insert(db,'studio_work_items',{company_id:company,project_id:p.id,shot_id:u.id,logical_key:'D010:generation',task_id:task.id,stage:'generation',role_key:'comp',execution:'creative'});
   await insert(db,'studio_role_bindings',{company_id:company,role_key:'comp',human_id:producer});
   const connectionId=randomUUID(),requestId=randomUUID(),providerJobId=randomUUID(),jobId=randomUUID(),outputId=randomUUID(),archiveId=randomUUID(),storageId=randomUUID(),bindingId=randomUUID(),fileId=randomUUID(),versionId=randomUUID(),uploadId=randomUUID();
   ...
   await insert(db,'project_storage_files',{id:fileId,company_id:company,project_id:p.id,binding_id:bindingId,name:'output.'+spec.format,name_key:'output.'+spec.format,created_by:producer});
   ...
   const input={clientId:randomUUID(),revision:1,workItemId:w.id,archiveId,name:'Original generated media',notes:'Reviewed registration request'};
