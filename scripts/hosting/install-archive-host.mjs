...
export async function installArchiveHost(bundlePath,expectedSha256){
 ...
 await mkdir(plan.release,{mode:0o755});await copyArchiveFiles(resolve(bundlePath),plan.release,bundle.files);await writeFile(join(plan.release,'bundle.json'),await archiveHostRead(join(resolve(bundlePath),'bundle.json')),{flag:'wx',mode:0o444});await verifyArchiveTree(plan.release,bundle.files,{trusted:true,extra:['bundle.json']});
...
export async function acceptArchiveHostQualification(evidencePath,expectedEvidenceSha256,previousReceiptSha256){
 ...
 const state=Object.fromEntries(command('/usr/bin/systemctl',['show','coatria-archive-qualify.service','--property=ActiveState,Result,ExecMainStatus,InvocationID']).split('\n').map(line=>line.split('=')));if(state.ActiveState!=='inactive'||state.Result!=='success'||state.ExecMainStatus!=='0')archiveHostFailure();
 ...
 if(report.qualified!==true||report.bundleSha256!==host.bundleSha256||report.bootId!==bootId||report.invocationId!==state.InvocationID||report.noProviderCalls!==true||report.testsPassed!==10||report.formatsPassed!==7)archiveHostFailure();
