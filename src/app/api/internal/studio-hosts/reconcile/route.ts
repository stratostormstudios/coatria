...
import {reconcileStudioHostProvisions} from '@/lib/studio-host-provisioning';
...
export async function GET(request:Request){
 ...
 try{
  // A small batch prevents one invocation from holding provider requests open
  // across the next cron tick. Every provision has its own durable lease.
  return json(await reconcileStudioHostProvisions(2));
