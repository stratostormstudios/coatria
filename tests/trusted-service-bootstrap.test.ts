...
function fixture(service='gateway'){
 ...
 return {manifest,settings,configuration};
}
...
test('gateway child receives its dedicated role and vault, never Vercel or lifecycle credentials',()=>{
 const {manifest,settings}=fixture(),result=trustedServiceChildEnvironment(manifest,settings);assert.deepEqual(Object.keys(result.env).sort(),['APP_URL','COATRIA_HOSTING_KEYRING','DATABASE_URL','HOME','HOST','LANG','NODE_ENV','PATH','PORT'].sort());assert.equal(result.env.DATABASE_URL,settings.DATABASE_URL);assert.doesNotMatch(JSON.stringify(result.env),/synthetic-vercel-token|synthetic-lifecycle-secret|NODE_OPTIONS|HTTP_PROXY/);
