...
The application closes the reviewed creative change-request loop for image, video and audio projects. Production source `550140f` has migrations 001–034 and matching application runtime grants. Candidate `120c6a8` has passed CI, the raw Runpod storage conformance test and an isolated-preview onboarding test. [RELEASE_STATUS.md](RELEASE_STATUS.md) is the authority for deployment and validation evidence. Archive and storage-gateway execution remain disabled pending their separate service roles and actual host qualification; the complete provider-to-client media pilot is still outstanding.
...
The pilot permits at most 20 applied rounds and 200 saved plans per project, alongside the existing artifact and package limits. It does not authorize automatic generation, credit spending, file transfer, client messaging or acceptance. The application/schema rollout is complete and raw Runpod storage transport conformance has passed. The deployed archive/gateway services, their actual host qualification and the complete generation, independent review and client-response cycle remain operational acceptance steps. The authorized simulated reviewer/client exercise must remain explicitly labeled SIMULATED; it cannot establish genuine human QC or client acceptance.
