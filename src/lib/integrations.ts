...
export async function integrationRoute(request: Request, parts: string[], method: string): Promise<Response|null> {
  ...
  if(parts[2]==='agents') {
    ...
    if(parts.length===4&&method==='PATCH') {
      ...
      const agent=await memberMutation(member,true,async client=>{
        const current=(await client.query('SELECT * FROM agents WHERE id=$1 AND company_id=$2 FOR UPDATE',[agentId,companyId])).rows[0];if(!current)fail(404,'Agent not found.');if((await client.query('SELECT 1 FROM plugin_installations WHERE company_id=$1 AND agent_id=$2',[companyId,agentId])).rowCount)fail(409,'Manage this agent through its plugin installation.','PLUGIN_MANAGED');if(current.status==='revoked'&&(data.status!=='revoked'||Object.keys(data).length>1))fail(409,'Revoked credentials cannot be reactivated. Register a new agent.');
