...
import {AgentRunProvider,AskAgentDialog,ConversationRuns} from './AgentRuns';
...
function ScopedConversations({children}:{children:ReactNode}){
 ...
 return <AgentRunProvider><ConversationContext.Provider value={{...client,readPositions:readPositions.current,threads,openThread}}>{children}</ConversationContext.Provider></AgentRunProvider>;
}
...
function ConversationPane(p:ChatPaneProps){
 ...
 const inputId=useId(),selectorId=useId(),headingId=useId(),viewport=useRef<HTMLDivElement>(null),textarea=useRef<HTMLTextAreaElement>(null),[unseen,setUnseen]=useState(0),[ask,setAsk]=useState<{parentId:string|null;contextText?:string;initialPrompt:string}|null>(null);
 ...
 return <section className={s.pane+' '+s[presentation]} aria-labelledby={headingId} data-conversation-presentation={presentation}>
  ...
  {ask&&<AskAgentDialog key={channel+':'+ask.parentId} workspace={p.workspace} user={p.user} channel={channel} parentId={ask.parentId} contextText={ask.contextText} initialPrompt={ask.initialPrompt} onClose={()=>setAsk(null)}/>}
 </section>;
}

const reactionGlyphs={thumbsup:'👍',heart:'❤️',applause:'👏',laugh:'😄',idea:'💡',celebrate:'🎉'};
function MessageRow({message,userId,channel,store,onReply,onAsk}:{message:ConversationMessage;userId:string;channel:string;store:ConversationStore;onReply?:()=>void;onAsk?:()=>void}){
 ...
 return <article className={'message '+s.message+(own?' own':'')}>
  ...
  <div><div className={'message-meta '+s.messageMeta}><strong>{message.authorName||'Team member'}{own&&<span className={s.youLabel}>you</span>}{message.actor.kind==='agent'&&<span className={s.agentLabel}>AI agent</span>}</strong><time dateTime={message.createdAt} title={when(message.createdAt)}>{new Date(message.createdAt).toLocaleTimeString(undefined,{hour:'numeric',minute:'2-digit'})}</time>{message.editedAt&&!message.deletedAt&&<span className={s.edited}>edited</span>}</div>
   ...
   {!message.deletedAt&&!editing&&<div className={s.messageActions}><button type="button" aria-label="Add reaction" aria-expanded={picker} disabled={busy} onClick={()=>setPicker(value=>!value)}><SmilePlus size={14}/></button>{onReply&&<button type="button" onClick={onReply}><MessageSquare size={14}/>{message.replyCount?message.replyCount+' '+(message.replyCount===1?'reply':'replies'):'Reply'}</button>}{onAsk&&<button type="button" aria-label="Ask agent about this message" onClick={onAsk}><Bot size={14}/></button>}{own&&<><button type="button" aria-label="Edit message" disabled={busy} onClick={()=>{setBody(message.body);setRevision(message.revision);setEditing(true);setError('');}}><Pencil size={13}/></button><button type="button" aria-label="Delete message" disabled={busy} onClick={()=>setDeleting(true)}><Trash2 size={13}/></button></>}</div>}
