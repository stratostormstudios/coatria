...
export function generatedTechnicalMatch(detail:StudioGeneratedProjectDetail,artifact:StudioGeneratedArtifact){
 const work=detail.workItems.find(item=>item.id===artifact.workItemId),shot=detail.shots.find(item=>item.id===work?.shotId);
...
export function generatedReviewerReason(detail:StudioGeneratedProjectDetail,artifact:StudioGeneratedArtifact,userId:string,role:string):string|null{
 ...
 if(!work||work.stage!=='generation'||work.execution!=='creative')return 'The exact generation work is unavailable. Refresh the project.';
...
export function generatedPackageSelection(detail:StudioGeneratedProjectDetail):{artifacts:StudioGeneratedArtifact[];reason:string|null}{
 ...
 const finals=qc.map(work=>detail.workItems.find(candidate=>candidate.id===work.dependencies[0]));
 if(qc.some((work,index)=>work.dependencies.length!==1||!finals[index]||finals[index]!.stage!=='generation'||finals[index]!.execution!=='creative'||finals[index]!.shotId!==work.shotId)||new Set(finals.map(work=>work!.id)).size!==detail.shots.length)return empty('Each QC task must depend on its exact generation final.');
 const artifacts:StudioGeneratedArtifact[]=[];
 for(const work of finals){
  const artifact=detail.artifacts.filter(item=>item.workItemId===work!.id).sort((a,b)=>b.version-a.version)[0];
