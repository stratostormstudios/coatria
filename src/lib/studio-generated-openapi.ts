...
export function studioGeneratedOpenApiSchemas(legacy:Record<string,Schema>):Record<string,Schema>{
 ...
 const packageReview=object({...legacy.StudioReview.properties,contractVersion:{const:2},...evidence,decision:{const:'approved'},technicalQc:{const:true}});
 ...
 const roundIdentity=object({roundId:uuid,number:{type:'integer',minimum:1,maximum:20},planSha256:sha});
 packageManifest.properties.revisionRound=roundIdentity;
 const delivery=object({...legacy.StudioDelivery.properties,roundId:nullable(uuid),roundNumber:{type:'integer',minimum:0,maximum:20},manifest:ref('StudioGeneratedPackageManifest')},Object.keys(legacy.StudioDelivery.properties));
 const roundScope=object({roundId:nullable(uuid),number:{type:'integer',minimum:0,maximum:20},planSha256:nullable(sha),workItemIds:{type:'array',items:uuid},finals:{type:'array',minItems:1,maxItems:100,items:object({unitId:uuid,generationWorkItemId:uuid,qcWorkItemId:uuid,carry:nullable(object({artifactId:uuid,reviewId:uuid,storageVersionId:uuid,manifestSha256:sha,fileSha256:sha}))})}});
 return {
