...
export async function generatedFollowupRunAuthority(db:PoolClient,companyId:string,run:Row):Promise<boolean>{
 ...
 try{const p=await policyAuthority(db,companyId,receipt);if(receipt.coordinator_agent_id===receipt.specialist_agent_id&&(await db.query('SELECT status FROM agent_runs WHERE company_id=$1 AND id=$2',[companyId,receipt.parent_run_id])).rows[0]?.status!=='succeeded')return false;await evidence(db,companyId,receipt.project_id,receipt.work_item_id,receipt.archive_id,receipt);await deadline(db,companyId,run,p.expiresAt);return true;}catch(error){if(!(error instanceof ApiError)||error.status>=500)throw error;return false;}
}
