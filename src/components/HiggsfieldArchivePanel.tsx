...
type Props={companyId:string;projectId:string;isAdmin:boolean;selection:ArchiveSelection|null;onClose:()=>void};
...
export function HiggsfieldArchivePanel(props:Props){return <ArchivePanel key={`${props.companyId}:${props.projectId}:${props.selection?.outputId??'history'}`} {...props}/>;}
function ArchivePanel({companyId,projectId,isAdmin,selection,onClose}:Props){
 ...
 const refresh=useCallback(async(folder:string|null=null,after?:string)=>{
  read.current?.abort();const controller=new AbortController(),ticket=++sequence.current;read.current=controller;setLoading(true);setError('');setReview(null);setConsent(false);
  ...
  try{
   ...
   const [archives,files,detail]=await Promise.all([
    ...
    api<StudioProjectDetail>(projectBase,'GET',undefined,{signal:controller.signal})
   ]);
   if(mounted.current&&ticket===sequence.current){setPage(previous=>({...archives,archives:after&&previous?[...previous.archives,...archives.archives.filter(item=>!previous.archives.some(old=>old.id===item.id))]:archives.archives}));setListing(files);setProjectRevision(detail.project.revision);setParentId(folder);setFileId('');}
 ...
 async function inspect(archive:HiggsfieldArchive){
  const [fresh,files,detail]=await Promise.all([api<{archive:HiggsfieldArchive}>(`${base}/${archive.id}`),api<ProjectStorageListing>(`${projectBase}/files?limit=1`),api<StudioProjectDetail>(projectBase)]);
  if(mounted.current){replace(fresh.archive);setReview(fresh.archive);setReviewBinding(files.binding);setProjectRevision(detail.project.revision);setConsent(false);setHours(2);}
 }
 ...
 return <section className={s.panel} aria-label="Project file archives" aria-busy={locked}>
  ...
  {selection&&listing&&<form className={s.form} onSubmit={event=>{event.preventDefault();void act(propose);}}>
   ...
   {!binding?<p className={s.notice}>Open this project’s Files tab to connect storage before preparing an archive.</p>:<>
    ...
    <button className="button primary small" disabled={locked||!name.trim()||binding.connection.status!=='configured'}>Prepare archive destination</button>
   </>}
  </form>}
