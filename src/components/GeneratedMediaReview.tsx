...
function ReviewWorkspace({p,detail,onReload}:Props){
 ...
 return <div className={s.workspace}>
  ...
  <div className={s.layout}><section className={s.stack} aria-label="Registered generated outputs">
   ...
   {detail.deliveries.map(delivery=><article className={s.package} key={delivery.id}><header><h4>{delivery.name}</h4><Badge>Not transferred</Badge></header><p>{delivery.note}</p><small>Prepared {when(delivery.createdAt)}</small><button className="button secondary small" onClick={()=>packageDownload(delivery)}><Download size={14}/> Download package JSON</button><p className={s.small}>Internal manifest only · external client sharing is unavailable for generated-media projects.</p></article>)}
