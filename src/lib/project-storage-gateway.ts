...
export function createProjectStorageGateway(options:{providerFactory?:ProviderFactory;allowedOrigins?:string[]}={}){
 ...
 async function handle(request:Request):Promise<Response>{
  ...
  try{
   ...
   if(url.pathname==='/health'&&request.method==='GET'){const ready=await transaction(async client=>Number((await client.query("SELECT count(*)::int AS count FROM schema_migrations WHERE name IN ('027_project_storage.sql','032_studio_generated_client_delivery.sql','033_studio_generated_revisions.sql')")).rows[0].count)===3);return json({service:'coatria-storage-gateway',status:ready?'ready':'schema_required',schemaVersion:3},ready?200:503);}
