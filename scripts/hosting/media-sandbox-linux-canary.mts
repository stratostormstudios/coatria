...
async function main(config:Config){
 ...
 try{
  ...
  const events:MediaSandboxExitEvidence[]=[];const sandbox=await createLinuxMediaSandbox({...config.profiles.conformance,cgroupRoot:config.cgroupRoot,onExitEvidence:event=>events.push(event)});assert.equal(events.length,2);assert.ok(events.every(event=>event.drained));
  ...
  const realEvents:MediaSandboxExitEvidence[]=[];const real=await createLinuxMediaSandbox({...config.profiles.real,cgroupRoot:config.cgroupRoot,onExitEvidence:event=>realEvents.push(event)});
 ...
 }catch(error){report.failureCode=code(error);throw error;}finally{if(listener)await closed(listener);await writeFile(join(config.evidence,'qualification.json'),JSON.stringify(report,null,2)+'\n',{mode:0o600});}
