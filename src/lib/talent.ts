...
export async function talentRoute(request:Request,parts:string[],method:string):Promise<Response|null> {
  ...
  if(parts[0]==='opportunities'&&parts.length===3&&parts[2]==='apply'&&method==='POST') {
    ...
    const application=await transaction(async client=>{
      const target=(await client.query("SELECT company_id FROM openings WHERE id=$1 AND status='published'",[openingId])).rows[0];if(!target)fail(404,'This opening is not accepting applications.');
      const source=data.agentId?(await client.query('SELECT company_id FROM agents WHERE id=$1 AND created_by=$2',[data.agentId,user.id])).rows[0]:null;
      if(data.agentId&&!source)fail(403,'You can only apply with an active agent you are authorized to sponsor.');
      const companies=[...new Set([target.company_id,...(source?[source.company_id]:[])])].sort();
      await client.query('SELECT id FROM companies WHERE id=ANY($1::uuid[]) ORDER BY id FOR KEY SHARE',[companies]);
      if(source) {
        const sponsor=(await client.query("SELECT role FROM memberships WHERE company_id=$1 AND user_id=$2 AND role IN ('owner','admin') FOR SHARE",[source.company_id,user.id])).rows[0];
        if(!sponsor)fail(403,'You no longer have permission to sponsor this company agent.');
      }
      ...
      if(data.agentId) {
        const agent=(await client.query("SELECT id FROM agents WHERE id=$1 AND created_by=$2 AND status='active' FOR SHARE",[data.agentId,user.id])).rows[0];if(!agent)fail(403,'You can only apply with an active agent you sponsor.');
  ...
  if(parts[2]==='applications'&&parts.length===4&&method==='PATCH') {
    ...
    const result=await memberMutation(member,true,async client=>{
      ...
      const invitation=data.status==='accepted'?await createInvitation(client,member,'member',null,request,current.user_id):null;
