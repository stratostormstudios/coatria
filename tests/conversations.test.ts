...
test('durable conversations preserve identity, ordering, retries, privacy and legacy messages',{skip:!emulator&&!local,timeout:120000},async t=>{
 ...
 if(emulator){const {PGlite}=await import('@electric-sql/pglite');const {PGLiteSocketServer}=await import('@electric-sql/pglite-socket');const db=await PGlite.create();for(const name of(await readdir('database')).filter(name=>/^\d.*\.sql$/.test(name)&&Number(name.slice(0,3))<7).sort())await db.exec(await readFile(resolve('database',name),'utf8'));const server=new PGLiteSocketServer({db,host:'127.0.0.1',port:0,maxConnections:1});await server.start();process.env.DATABASE_URL=`postgresql://postgres:postgres@${server.getServerConn()}/postgres`;stop=async()=>{await server.stop();await db.close();};}else process.env.DATABASE_URL=url!;
 ...
 try{
  ...
  if(emulator)for(const name of(await readdir('database')).filter(name=>/^\d.*\.sql$/.test(name)&&Number(name.slice(0,3))>=7).sort())await query(await readFile(resolve('database',name),'utf8'));
