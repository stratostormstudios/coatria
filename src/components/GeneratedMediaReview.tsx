...
import {assertGeneratedDownload,generatedArchiveReason,generatedBytes,generatedCarriedArtifact,generatedCurrentDeliveries,generatedEvidenceWork,generatedLimitationText,generatedManifestPath,generatedPackageSelection,generatedProjectPath,generatedRegistrationReason,generatedReviewerReason,generatedTechnicalMatch,generatedVersionAccessPath} from '@/lib/generated-media-ui';
...
import {GeneratedRevisionPlan} from './GeneratedRevisionPlan';
...
function ReviewWorkspace({p,detail,onReload}:Props){
 const [register,setRegister]=useState(false),[review,setReview]=useState<StudioGeneratedArtifact|null>(null),[packaging,setPackaging]=useState(false),[history,setHistory]=useState(false);
 ...
 const round=detail.generatedRound,evidenceWork=generatedEvidenceWork(detail),currentArtifacts=detail.artifacts.filter(artifact=>!round?.roundId||detail.workItems.some(work=>work.id===artifact.workItemId)||generatedCarriedArtifact(detail,artifact.id)),shown=history?detail.artifacts:currentArtifacts;
 const clientBlock=detail.project.status==='delivered'?'This project has an authenticated client acknowledgement. Existing invitations and receipts remain below.':!generatedCurrentDeliveries(detail).some(delivery=>delivery.status==='prepared')?'Prepare an approved internal package for the current round first.':!detail.workItems.some(work=>work.stage==='delivery')||detail.workItems.some(work=>work.status!=='done')?'Accept every production, QC and delivery-handoff task before inviting the client.':detail.project.gates.production?.decision!=='approved'?'Production approval is required before client access.':null;
 ...
 return <div className={s.workspace}>
  ...
  <GeneratedRevisionPlan p={p} detail={detail} onReload={onReload}/>
  {admin&&registrationBlock&&<p className={s.muted}>{registrationBlock}</p>}
  {round?.roundId&&<div className={s.actions}><Badge>Revision round {round.number}</Badge><button className="button secondary small" onClick={()=>setHistory(!history)}>{history?'Show current round outputs':'Include earlier round outputs'}</button><p className={s.small}>Kept versions retain their original file and review evidence.</p></div>}
  <div className={s.layout}><section className={s.stack} aria-label="Registered generated outputs">
   {!shown.length?<div className={s.empty}><FileCheck2 size={25}/><h3>No verified output registered yet{round?.roundId?' for this round':''}</h3><p>Use the generation panel to archive a completed provider output into project storage. Once the archive is verified, register it against its exact generation task here.</p></div>:shown.map(artifact=><article key={artifact.id} className={s.card}>
    <header className={s.cardHeader}><div className={s.mediaTitle}><span className={s.mediaIcon}>{artifact.mediaKind==='image'?<ImageIcon size={21}/>:artifact.mediaKind==='video'?<Video size={21}/>:<Music2 size={21}/>}</span><div><h3>{artifact.name}</h3><p>Version {artifact.version} · {evidenceWork.find(work=>work.id===artifact.workItemId)?.title??'Generation task'}</p>{generatedCarriedArtifact(detail,artifact.id)&&<p>Kept unchanged from the approved package</p>}</div></div><Badge tone={artifact.reviewStatus==='changes_requested'?'warning':''}>{humanize(artifact.reviewStatus)}</Badge></header>
   ...
   {detail.deliveries.map(delivery=><article className={s.package} key={delivery.id}><header><h4>{delivery.name}</h4><Badge>Not transferred</Badge></header><p>{delivery.note}</p><small>{delivery.roundNumber?`Revision round ${delivery.roundNumber}`:'Original production'} · Prepared {when(delivery.createdAt)}{(delivery.roundId??null)!==(round?.roundId??null)?' · Earlier package preserved':''}</small><button className="button secondary small" onClick={()=>packageDownload(delivery)}><Download size={14}/> Download package JSON</button><p className={s.small}>This internal manifest preserves its preparation state. Client invitations, access and responses are recorded separately below.</p></article>)}
  ...
  <StudioClientDeliveries companyId={p.company.id} projectId={detail.project.id} projectRevision={detail.project.revision} deliveries={detail.deliveries} currentRoundId={round?.roundId??null} admin={admin} creationBlocked={clientBlock} onChanged={onReload}/>
...
function ApprovedTarget({detail,workItemId}:{detail:StudioGeneratedProjectDetail;workItemId:string}){
 const spec=detail.project.spec,work=generatedEvidenceWork(detail).find(item=>item.id===workItemId),unit=detail.shots.find(item=>item.id===work?.shotId);
...
function GeneratedPackageEditor({p,detail,onClose,onSaved}:{p:WorkspaceProps;detail:StudioGeneratedProjectDetail;onClose:()=>void;onSaved:()=>void|Promise<void>}){
 const [revision]=useState(detail.project.revision),[name,setName]=useState(`${detail.project.name}${detail.generatedRound?.roundId?` · revision ${detail.generatedRound.number}`:''} · approved finals`.slice(0,160)),[note,setNote]=useState(''),[confirm,setConfirm]=useState(false),[attempt,setAttempt]=useState<Record<string,unknown>|null>(null),[attemptView,setAttemptView]=useState<StudioGeneratedProjectDetail|null>(null);
 ...
 return <Modal title="Prepare an internal media package" description="Include the exact latest approved final for every deliverable. No media is transferred." onClose={()=>{if(!mutation.busy)onClose();}} wide><form className="form" onSubmit={event=>{event.preventDefault();if(!attempt&&(selection.reason||!confirm||!name.trim()||!note.trim()||detail.project.revision!==revision))return;const body=attempt??{revision,name:name.trim(),artifactIds:selection.artifacts.map(artifact=>artifact.id),note:note.trim()};if(!attempt)setAttemptView(structuredClone(detail));setAttempt(body);void mutation.mutate(generatedProjectPath(p.company.id,detail.project.id)+'/deliveries',body,onSaved);}}>
  <fieldset disabled={mutation.busy||!!attempt}><Field label="Package name"><input required maxLength={160} value={name} onChange={event=>setName(event.target.value)}/></Field><h3>Exact approved finals</h3>{view.generatedRound?.roundId&&<p className={s.small}>Complete revision package: new independently reviewed outputs and exact kept versions.</p>}{selection.reason&&<p className={s.notice}>{selection.reason}</p>}<ul className={s.finalList}>{selection.artifacts.map(artifact=><li key={artifact.id}><strong>{artifact.name} · v{artifact.version}</strong><small>{generatedEvidenceWork(view).find(work=>work.id===artifact.workItemId)?.title} · {artifact.mediaKind}{generatedCarriedArtifact(view,artifact.id)?' · Kept unchanged':''}</small><code>File SHA-256 {artifact.file.sha256}</code><code>Storage version {artifact.provenance.storageVersionId}</code></li>)}</ul>
   ...
   <label className={s.checkbox}><input type="checkbox" required checked={confirm} onChange={event=>setConfirm(event.target.checked)}/><span>I reviewed the included final versions. This is an internal manifest with status “not transferred”.</span></label><Notice>The package records current independent review receipts. After the delivery handoff is accepted, an administrator can invite the client to this exact package below. File access and the client’s authenticated response are recorded separately.</Notice>
